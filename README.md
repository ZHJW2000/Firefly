...
| 阶段 | 能力 | 说明 |
|:---:|---|---|
| ① 资产收集 | FOFA 优先 + OneForAll | FOFA 按 org/title/domain 查询互联网暴露资产（需配置账号）；OneForAll 对域名目标补充子域收集；两者结果合并去重 |
...
- 📋 批量测绘：目标框每行一个域名（或从文件导入），一次跑完并输出合并报表
...
2. 双击 `AssetRadar.exe`
3. 输入目标域名（**每行一个支持批量**，或「导入目标列表」从文件载入）→ 勾选授权确认 → 点击 **开始测绘**
...
| FOFA 优先收集 | 开启 | 关闭则仅用 OneForAll；FOFA 账号在「FOFA 设置…」配置 |
| FOFA 资产上限 | 2000 | 控制 API 积分消耗 |
| FOFA 验证扫描 | 关闭 | 开启后对 FOFA 资产 IP 再跑 Nmap 复核 |
